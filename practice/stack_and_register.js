...
// If we do another MULT at this point, the stack is mutated to [3], and the
...
// P: given a program (string), process the tokens in that program per
//    instructions above
//  assume valid program
//  init stack to [], register to 0
// E: Given
// D: array (stack)
// A:
// - declare function minilangThread that takes no params,
//    - initialize stack and register
//    - define object containing arrow functions for all program commands
//      - including POPULATE_REGISTER to handle number input
//    - return function that takes 2 params -> command, arg(optional-null)
//      - if command can be parsed as int, call
//        functionObject[POPULATE_REGISTER, int]
//      - else functionObject[command]
//
// - declare function minilang that takes one param -> program
//    - parse program into tokens
//    - initialize program
//    - loop over tokens, passing each to program

function minilangThread() {
  const stack = [];
  let register = 0;

  const minilangFunctions = {
    POPULATE_REGISTER: (num) => (register = num),
  ...
  return function executeCommand(command) {
    if (Number(command)) {
      minilangFunctions['POPULATE_REGISTER'](Number(command));
    } else {
      minilangFunctions[command]();
    }
  };
}
...
function minilang(program) {
  ...
  const myMinilang = minilangThread();

  tokens.forEach(token => myMinilang(token));
}
