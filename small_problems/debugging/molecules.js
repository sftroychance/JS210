...
// molecule. Fortunately, this is relatively straightforward. For each element
// in the molecule, we need to know two things:
...
// The number of atoms of that element in the molecule. We can read this
// directly from the string representation of the molecule.

// For example, Geosmin has the string representation C12H22O, so it has
// 12 C atoms, 22 H atoms, and 1 O atom,
// summing up to 12*4 + 22*1 + 1*6 = 76 valence electrons.

// We decided to write our function valenceOfMolecule such that it expects
// string representations of each element in the molecule as input, e.g.
...
// The problem here is that when `forEach()` is called on `arguments`, this
// will fail because `arguments` is an array-like object, not an array, and
// thus cannot call `forEach()`

// The solution is to collect the arguments using `...args` as the
// parameter; this is an array and can be iterated using `forEach()`
