// the issue with the original code was that random is returning an array
// but dishName and dish are defined using '+', which concatenates but
// after coercing each array into a string. Calling `join()` on that
// string result would cause an error.

// the solution was to use the spread operator to define dishName and dish
// as arrays containing the random values, then allowing `join()` to work
// as expected
